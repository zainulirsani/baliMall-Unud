 ...
 *
 * Modified to meet requirements by Gs Mahardika
 * See comments with "additional changes" in this file
 *
 * Updated on 21-11-2020 to make this class works with PHP 7.4
 */
...
class UploadHandler
{
    ...
    // Check the real file type (additional changes #4)
    protected $image_type_check = [IMAGETYPE_GIF, IMAGETYPE_JPEG, IMAGETYPE_PNG];
    protected $video_type_check = ['video/mp4', 'video/webm', 'video/ogg'];
    protected $file_type_check = ['application/msword', 'application/vnd.ms-excel', 'application/pdf'];
    protected $payment_type_check = ['image/gif', 'image/jpeg', 'image/png', 'application/pdf'];
            ...
            'image_versions' => [
                ...
                */
                'thumbnail' => [
                    ...
                    'max_width' => 80,
                    // either specify height, or set to 0. Then height is automatically adjusted - keeping aspect ratio to a specified max_width.
                    'max_height' => 80
                ]
            ],
            'print_response' => true,
            // Overwrite existing file or not (additional changes #2)
            'overwrite_file' => false
        ];

        if ($options) {
            // dd($options + $this->options);
    ...
    protected function get_full_url(): string
    {
        ...
        return ($https ? 'https://' : 'http://') . (!empty($_SERVER['REMOTE_USER']) ? $_SERVER['REMOTE_USER'] . '@' : '') . ($_SERVER['HTTP_HOST'] ?? ($_SERVER['SERVER_NAME'] . ($https && $_SERVER['SERVER_PORT'] === 443) || $_SERVER['SERVER_PORT'] === 80 ? '' : ':' . $_SERVER['SERVER_PORT'])) . substr($_SERVER['SCRIPT_NAME'], 0, strrpos($_SERVER['SCRIPT_NAME'], '/'));
    }
    ...
    {
        return ($this->options['user_dirs']) ? $this->get_user_id() . '/' : '';
    }
    ...
    protected function get_file_object($file_name): ?stdClass
    {
        if ($this->is_valid_file_object($file_name)) {
            ...
            foreach ($this->options['image_versions'] as $version => $options) {
                if (!empty($version) && is_file($this->get_upload_path($file_name, $version))) {
    ...
    protected function validate($uploaded_file, $file, $error, $index): bool
    {
        ...
        // Set post_max_size from options value because the original post_max_size
        // sometimes always smaller than the content_length (additional changes #5)
        if ((int) $this->options['max_file_size'] > $post_max_size) {
            $post_max_size = (int) $this->options['max_file_size'];
        }
        ...
        // Check the real file type (additional changes #4)
        switch ($this->options['param_name']) {
            case 'file_image':
                $next = 'image';
                $allowed = 'image_type_check';
                break;
            case 'file_video':
                $next = 'video';
                $allowed = 'video_type_check';
                break;
            case 'file_file':
                $next = 'file';
                $allowed = 'file_type_check';
                break;
            case 'file_payment':
                $next = 'payment';
                $allowed = 'payment_type_check';
                break;
            default:
                $next = 'default';
                $allowed = '';
        ...
        if ($next === 'image') {
            if (!in_array(exif_imagetype($uploaded_file), $this->{$allowed}, false)) {
                $file->error = $this->get_error_message('accept_file_types');

                return false;
            }
        } elseif ($next === 'video' || $next === 'file' || $next === 'payment') {
            if (!in_array(mime_content_type($uploaded_file), $this->{$allowed}, false)) {
                $file->error = $this->get_error_message('accept_file_types');

                return false;
            }
        } elseif (!preg_match($this->options['accept_file_types'], $file->name)) {
            $file->error = $this->get_error_message('accept_file_types');
        ...
        if (is_int($this->options['max_number_of_files'])
            ...
            && !is_file($this->get_upload_path($file->name))) { // Ignore additional chunks of existing files:
    ...
    protected function up_count_name($name)
    {
        return preg_replace_callback('/(?:(?: \(([\d]+)\))?(\.[^.]+))?$/', [$this, 'up_count_name_callback'], $name, 1);
    }

    protected function get_unique_filename($file_path, $name, $size, $type, $error, $index, $content_range)
    {
        while (is_dir($this->get_upload_path($name))) {
            $name = $this->up_count_name($name);
        }

        // Keep an existing filename if this is part of a chunked upload:
        //$uploaded_bytes = $this->fix_integer_overflow((int) $content_range[1]); // Not working in PHP 7.4
        $uploaded_bytes = $this->fix_integer_overflow(isset($content_range[1]) ? (int) $content_range[1] : 0);
    ...
    protected function fix_file_extension($file_path, $name, $size, $type, $error, $index, $content_range): string
    {
    ...
    protected function get_file_name($file_path, $name, $size, $type, $error, $index, $content_range)
    {
        ...
        if ($this->options['overwrite_file']) {
            // Replace space with underscore (additional changes #3)
            return str_replace([' ', '#'], ['_', ''], $name);
        }

        $unique_name = $this->get_unique_filename(
            $file_path,
    ...
    protected function get_scaled_image_file_paths($file_name, $version): array
    {
        $file_path = $this->get_upload_path($file_name);
    ...
    protected function gd_image_flip($image, $mode)
    {
    ...
    protected function gd_orient_image($file_path, $src_img): bool
    {
        ...
        switch ($orientation) {
            case 2:
                $new_img = $this->gd_image_flip($src_img, defined('IMG_FLIP_VERTICAL') ? IMG_FLIP_VERTICAL : 2);
                break;
            case 3:
                $new_img = imagerotate($src_img, 180, 0);
                break;
            case 4:
                $new_img = $this->gd_image_flip($src_img, defined('IMG_FLIP_HORIZONTAL') ? IMG_FLIP_HORIZONTAL : 1);
                break;
            case 5:
                $tmp_img = $this->gd_image_flip($src_img, defined('IMG_FLIP_HORIZONTAL') ? IMG_FLIP_HORIZONTAL : 1);
            ...
            case 6:
                ...
                break;
            case 7:
                $tmp_img = $this->gd_image_flip($src_img, defined('IMG_FLIP_VERTICAL') ? IMG_FLIP_VERTICAL : 2);
                $new_img = imagerotate($tmp_img, 270, 0);
                imagedestroy($tmp_img);
                break;
            case 8:
    ...
    protected function gd_create_scaled_image($file_name, $version, $options): bool
    {
    ...
    protected function create_scaled_image($file_name, $version, $options): bool
    {
        if ($this->options['image_library'] === 2) {
    ...
    protected function destroy_image_object($file_path): bool
    {
        if ($this->options['image_library'] && extension_loaded('imagick')) {
    ...
    ): stdClass
    {
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            // Make sure file permission is set correctly (additional changes #1)
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                ...
                if ($append_file) {
                    ...
                    $this->sftpUploader->upload($file_path, $_ENV['SFTP_REMOTE_DIR'] . $upload_dir, $file->name);
                }
            } else {
                // Non-multipart uploads (PUT method support)
                file_put_contents($file_path, fopen($this->options['input_stream'], 'r'), $append_file ? FILE_APPEND : 0);
            }
            // dd('test',$file->name, $file_path, $upload_dir);
