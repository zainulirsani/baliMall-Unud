...
use App\EventListener\ListenerInterface;
...
{
    protected function appGenericEventDispatcher(GenericEvent $event, string $eventName, ListenerInterface $listener): void
    {
    ...
    protected function deniedManyAccess(array $roles): void
    {
        $invalid = 0;

        foreach ($roles as $role) {
            try {
                $this->deniedAccess($role);
            } catch (AccessDeniedException $e) {
                $invalid++;
            }
        }

        if ($invalid === count($roles)) {
            throw new AccessDeniedException($this->getTranslator()->trans('message.error.403'));
        }
    }
